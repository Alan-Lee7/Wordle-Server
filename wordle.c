...
void check_guess(const char *actual_word, const char *guess, char *result) {
    ...
    int *actual_freq = (int *)calloc(26, sizeof(int)); // Frequency of each letter in actual_word
    //Frequency of letters in guess
    int *guess_freq = (int *)calloc(26, sizeof(int));  // Frequency of each letter in guess
    ...
    for (i = 0; i < length; i++) {
        if (*(guess + i) == *(actual_word + i)) {
            *(result + i) = toupper((unsigned char)*(guess + i)); // Correct position
        } else {
